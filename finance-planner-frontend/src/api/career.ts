...
export function getRecommendations() {
  return request.get<ApiResponse<CareerRecommendation[]>>('/career/recommendations', {
    timeout: 60000
  })
}
...
export function addCareerIncome(planId: number, data: AddCareerIncomeRequest) {
  return request.post<ApiResponse<CareerIncome>>(`/career/plans/${planId}/income`, data)
}

export function getCareerIncomeHistory(planId: number) {
  return request.get<ApiResponse<CareerIncome[]>>(`/career/plans/${planId}/income`)
}

export function generateStartupPlan(planId: number) {
  return request.post<ApiResponse<string>>(`/career/plans/${planId}/startup-plan`, null, {
    timeout: 60000
  })
}
