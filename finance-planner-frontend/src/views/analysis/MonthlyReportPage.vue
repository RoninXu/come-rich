    ...
    <n-spin :show="loading">
      <PageHeader title="月度报表">
        <template #actions>
          ...
          />
          <n-button @click="handleExportReport" :loading="exporting">
...
import { NSpin, NGrid, NGi, NButton, NIcon, NDatePicker, useMessage } from 'naive-ui'
...
function initCharts() {
  nextTick(() => {
    if (expensePieRef.value) expensePieChart = echarts.init(expensePieRef.value)
    if (dailyTrendRef.value) dailyTrendChart = echarts.init(dailyTrendRef.value)
  })
}
...
function updateExpensePieChart() {
  ...
  expensePieChart.setOption({
    ...
    series: [{
      ...
      itemStyle: { borderRadius: 10, borderColor: 'var(--cr-bg-card)', borderWidth: 2 },
      label: { show: false },
...
function updateDailyTrendChart() {
  ...
  dailyTrendChart.setOption({
    ...
    xAxis: { type: 'category', boundaryGap: false, data: dates.length > 0 ? dates : ['暂无数据'] },
    yAxis: { type: 'value' },
    series: [
      { name: '收入', type: 'line', smooth: true, data: incomeData, itemStyle: { color: '#34C759' } },
      { name: '支出', type: 'line', smooth: true, data: expenseData, itemStyle: { color: '#FF3B30' } }
    ]
  }, true)
}
</script>
...
.monthly-report-page {
  ...
  .summary-card {
    ...
    &.income .card-value { color: var(--cr-success); }
    &.expense .card-value { color: var(--cr-error); }
    &.balance .card-value { color: var(--cr-primary); }
    &.savings .card-value { color: var(--cr-warning); }
  }
