    ...
    <n-spin :show="loading">
        ...
        >
                ...
                >
                  ...
                  <n-gi>
                    <div class="stat-item">
                      ...
                      <div class="stat-value">
                        ¥{{ formatNumber(goal.monthlySavingsNeeded) }}
                      </div>
                    </div>
                  </n-gi>
                </n-grid>
          ...
          <n-gi :span="8">
            <GlassCard>
              ...
              >
                <n-descriptions-item label="优先级">
                  <n-tag
                    :type="priorityTag(goal.priority)"
                    size="small"
                  >
            ...
            <GlassCard style="margin-top: 16px">
              ...
              <div
                v-if="aiPlan"
                class="ai-plan-content"
              >
                <h4>总结</h4>
                <p>{{ aiPlan.summary }}</p>
                <h4 v-if="aiPlan.steps.length > 0">
                  实施步骤
                </h4>
                <ol>
                  <li
                    v-for="(step, idx) in aiPlan.steps"
                    :key="idx"
                  >
                    {{ step }}
                  </li>
                </ol>
                <h4 v-if="aiPlan.tips.length > 0">
                  小贴士
                </h4>
                <ul>
                  <li
                    v-for="(tip, idx) in aiPlan.tips"
                    :key="idx"
                  >
                    {{ tip }}
                  </li>
      ...
      >
        ...
        >
          ...
          >
            <n-date-picker
              v-model:value="progressForm.recordDateTs"
              type="date"
              style="width: 100%"
            />
          </n-form-item>
        ...
        <template #action>
          <n-space justify="end">
            <n-button @click="showAddProgress = false">
              取消
            </n-button>
            <n-button
              type="primary"
    ...
    series: [
      {
        ...
        markLine: goal.value
          ? {
              data: [
                {
                  yAxis: Number(goal.value.targetAmount),
                  name: "目标",
...
function statusTag(status: number): "default" | "success" | "info" {
  const map: Record<number, "default" | "success" | "info"> = {
    1: "default",
    2: "success",
    3: "info",
  };
